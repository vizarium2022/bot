...
def get_main_keyboard():
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            [
                KeyboardButton(text="📁 Ссылка на каналы"),
    ...
    for name, cid in CHANNELS.items():
        try:
            ...
            kb.inline_keyboard.append([
                InlineKeyboardButton(text=f"🔗 Канал {name}", url=link.invite_link)
            ])
        except:
...
async def upload_button(message: types.Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        ...
        return
    await message.answer("📤 Отправьте файл. Он улетит во все 3 канала.")
...
async def unload_button(message: types.Message):
    ...
    await message.answer(
        "📥 *Как выгрузить файл:*\n\n"
        "1. Нажми «Ссылка на каналы» и введи пароль\n"
        "2. Зайди в нужный канал\n"
